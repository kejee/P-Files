...
> 🚀 **P-Files** 是一个轻量、高效、安全且具备全方位访问控制的独立文件分享系统。支持 **Docker 一键秒级部署**、**提取码取件**、**标准 Zip AES-256 加密压缩与解密还原**、**IP 离线高精度物理地区审计**、**5 项策略自由组合（单选/多选/全选）** 以及 **工业级防爆破与双模式阅后即焚**。
...
### 🗜️ 文件 AES-256 加密压缩与解密还原 (Zip Encryption & Restore)
> 支持标准 Zip AES-256 工业级加密压缩，后台明文展示并一键复制密码，支持保留原文件备份随时一键无损还原：

<p align="center">
  <img src="docs/images/08_aes256.png" alt="文件加密压缩与解密还原" width="95%" style="border-radius: 8px; box-shadow: 0 8px 30px rgba(0,0,0,0.5);">
</p>
...
- 🗜️ **标准 Zip AES-256 加密压缩与无损解密还原**：
  - **工业级 AES-256 加密**：采用标准 Zip AES-256 算法，Windows 11、macOS、7-Zip、Bandizip、WinRAR 原生完美支持；密码留空时生成标准普通 ZIP。
  - **后台明文呈现与一键复制**：列表清晰展示解压密码（如 `🔑 压缩密码: Abc@123`），一键快速复制发给访客。
  - **保留备份与无损还原**：加密时可选择保留原文件备份，支持随时在后台点击 **“🔓 恢复”** 一键切回原文件并自动物理销毁旧加密包；删除或即焚时彻底抹除双份文件。
  - **流式实时进度与并发热替换**：后台异步非阻塞逐块流式压缩，实时计算反馈 0%~100% 进度；压缩期间前台 423 友好拦截，压缩完毕瞬间原子替换。
